$(document).ready(function(){
  // Add user
  var departamentos = $('#dev_sel').select2();
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    bootbox.confirm("¿Realmente desea eliminar este usuario?", function(result) {
      if(result){
        $.ajax({
          url: 'manage_users_conf.php',
          type: 'POST',
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        for (var i = 0; i < len; i++) {
            ...
            user_dev.User_dev.push(response[i].departamentos.map((departamento) => {
              return departamento.id_device;
            }));
        ...
        console.log(user_dev.User_dev[0]);
        departamentos.val(user_dev.User_dev[0]).trigger('change');
